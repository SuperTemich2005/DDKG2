...
var State # defines current state, next button's props depend on it mostly. # Valid states are 'Def', 'Choice', 'Inv', 'Main', 'Chat', 'Move', 'Inter'
...
func _on_Next_pressed():
			...
			if get_parent().Dialogue[Cur].split("|").size() >= 3: # has animation def
				...
				if get_parent().Dialogue[Cur].split("|").size() >= 4:
					if get_parent().Dialogue[Cur].split("|")[3] != "---":
						...
						if get_parent().Dialogue[Cur].split("|")[3].split(" ")[0] == "START2":
							if $AudioStreamPlayer3.playing != true:
								$AudioStreamPlayer3.set_stream(load("res://sounds/"+get_parent().Dialogue[Cur].split("|")[3].split(" ")[1]+".ogg"))
	...
	if get_parent().Dialogue[Cur].split(" ")[0] == "JUMP":
		...
		_on_Next_pressed()
	if get_parent().Dialogue[Cur].split(" ")[0] == "EJUMP":
		print("EJUMP")
		Cur = int(get_parent().Dialogue[Cur].split(" ")[1])-1
		_on_Next_pressed()
		State = "Dialogue"
		$Back.hide()
		$Press.hide()
		$ShowCourtRecord.text = "Записи суда"
...
func _on_PresentEvidence_pressed():
	$Bubble.animation = "obj"
	$Bubble.show()
	$AudioStreamPlayer2.set_stream(load("res://sounds/obj_rus"+get_parent().va+".ogg"))
	$AudioStreamPlayer2.play()
	$hidebub.start()
	_on_ShowCourtRecord_pressed()


func _on_hidebub_timeout():
	$Bubble.hide()
	if $Bubble.animation == "obj":
		if Cur == int(get_parent().correct.split(" ")[0]) and save_file.get_value("Evidence",str(Selected)).split(";")[0] == get_parent().correct.split(" ")[1]:
			get_tree().change_scene(get_parent().correct.split(" ")[2])
		else:
			Cur = int(get_parent().correct.split(" ")[3])-1
			_on_Next_pressed()
			$CourtRecord/ColorOverlay.color = get_parent().BoxColor
			State = "Dialogue"
			$Back.hide()
			$Press.hide()
			$ShowCourtRecord.text = "Записи суда"
	elif $Bubble.animation == "hit":
		Cur = int(get_parent().Dialogue[Cur].split("|")[5])-1
		_on_Next_pressed()
		$CourtRecord/ColorOverlay.color = get_parent().BoxColor
		State = "Dialogue"
		$Back.hide()
...
func _on_Press_pressed():
	$Bubble.animation = "hit"
	$Bubble.show()
	$AudioStreamPlayer2.set_stream(load("res://sounds/hit_rus"+get_parent().va+".ogg"))
	$AudioStreamPlayer2.play()
	$hidebub.start()
